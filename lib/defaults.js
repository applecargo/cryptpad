...
Default.commonCSP = function (opt) {
    opt = opt || {};
    var domain = opt.main;
    var sandbox = opt.sandbox;
    var embed = opt.noEmbed? domain + (sandbox? ' ' + sandbox: ''): '*';
    ...
    return [
        ...
        "frame-ancestors " + embed, //*",
        //"frame-ancestors *",// + embed, //*",
        ""
    ];
};

Default.contentSecurity = function (opt) {
    return (Default.commonCSP(opt).join('; ') + "script-src 'self' resource: " + opt.main).replace(/\s+/g, ' ');
};

Default.padContentSecurity = function (opt) {
    return (Default.commonCSP(opt).join('; ') + "script-src 'self' 'unsafe-eval' 'unsafe-inline' resource: " + opt.main).replace(/\s+/g, ' ');
};
